  ...
  describe('GetBooksByCategoryAndNewRelease', () => {
    beforeEach(() => {
      (findByCategoryAndNewRelease as jest.Mock).mockResolvedValue({
        ...
        totalCount: newReleaseExistingBook.length,
      });
    });
    context('카테고리 id와 신간이 존재하면', () => {
      it('최신 신간 목록 도서 정보를 반환한다.', async () => {
        const newReleaseBooks = await getBooksByCategoryAndNewRelease(CATEGORY_ID, LIMIT, CURRENT_PAGE);
    ...
    context('카테고리 id가 올바르게 주어지지 않으면', () => {
      it('HttpException을 던져야 한다.', async () => {
        await expect(
          getBooksByCategoryAndNewRelease(notNewReleaseBook.categoryId, LIMIT, CURRENT_PAGE),
        ).rejects.toThrow(
